    ...
    if isinteger(str(a)):
        ...
        if ((a/100).is_integer()) & ((a/1000).is_integer()) & (a >= 100):
            ...
            if (int(a) >= 2000) & (int(a) <= 2009):
                ...
                if a < 30:
                    o = "twenty " + num2words(int(str(a)))
                else:
                    o = "nineteen " + num2words(int(str(a)))
                o = re.sub(' and','',o)
                o = re.sub('-',' ',o)
                return o
...
def date(a):
    ...
    for j in range(len(a)):
        try:
            ...
            if len(seg) == 1: # if 1-word date (e.g. 1900s, Aug27)
                segfors = re.findall(r"[0-9]+|[s]",str(inputdate[j]))
                seg_letter = re.sub(r"[^a-zA-Z]", "", seg[0]).lower()
                seg_number = re.sub(r"[^0-9]", "", seg[0])
                
                if len(segfors) == 1:
                    if seg_letter in list(month_sim.keys()) + month_full:
                        if (len(seg_number)==2 and 0<=int(seg_number)<=31):
                            output[j] = month_sim[seg_letter] + " " + num2words(seg_letter, ordinal=True)
                        elif len(seg_number)==4:
                            output[j] = month_sim[seg_letter] + " " + dateyear(seg_number)
                    elif (str(segfors[0])[0] in numbers):
                        if (int(segfors[0]) <= 1000) | (int(segfors[0]) >= 2100):
                ...
                if str(seg[1]) == 's':
                    ...
                    seg[0] = seg[0].lower()
                    seg_1_orig = str(seg[1])
                    seg[1] = re.findall(r"[0-9]+",str(seg[1]))[0]
                    if str(seg[0]) in month_sim:
                        if seg_1_orig[0] == "'": # if 2nd word is a year
                            if str(seg[1])[0] == '0':
                                output[j] = month_sim[str(seg[0])] + ' o ' + dateyear(seg[1][-1])
                                output[j] = re.sub('-',' ',output[j])
                            else:
                                output[j] = month_sim[str(seg[0])] + ' ' + dateyear(seg[1])
                                output[j] = re.sub('-',' ',output[j])
                                
                        else: # if 2nd word is a date
                            if int(str(seg[1])) <= 31:
            ...
            elif len(seg) == 3: # if 3-word date
                if str(seg[0])[0] in letters: # if 1st ch of the 1st word is a letter
                    seg[0] = re.findall(r"[a-zA-Z]+",str(seg[0]))[0]
                    seg[0] = seg[0].lower()
                    ...
                    if str(seg[0]) in weekdays:
                        ...
                        seg[1] = re.findall(r"[0-9]+",str(seg[1]))[0]
                        if str(seg[0]) in month_sim:
                            if str(seg[2])[0] == '0' or (str(seg[2])[0] == "'" and str(seg[2])[1] == '0'):                            
                                output[j] = month_sim[str(seg[0])]+ ' ' + num2words(int(str(seg[1])),ordinal = True) + ' o ' + dateyear(seg[2][-1])
                                output[j] = re.sub('-',' ',output[j])
                            elif str(seg[2])[0] == "'":                            
                                output[j] = month_sim[str(seg[0])]+ ' ' + num2words(int(str(seg[1])),ordinal = True) + ' ' + dateyear(seg[2][1:])
                                output[j] = re.sub('-',' ',output[j])
                            else:
                ...
                elif str(seg[0])[0] in numbers: # if 1st ch of the 1st word is a number
                    seg[0] = re.findall(r"[0-9]+",str(seg[0]))[0]
                    if str(seg[1])[0] in letters:
                        seg[1] = re.findall(r"[a-zA-Z]+",str(seg[1]))[0]
                        seg[1] = seg[1].lower()
                        ...
                        if str(seg[1]) in month_sim:
                            if (int(str(seg[0])) <= 31):
                                if(str(seg[2])[0] in numbers) or (len(seg[2])>1 and str(seg[2])[0]=="'" and str(seg[2])[1] in numbers):
                                    if len(str(seg[2])) == 2:
                                        if str(seg[2])[0] == '0':
                                        ...
                                        if str(seg[2])[0]=="'":
                                            if str(seg[2])[1]=="0":
                                                ...
                                                output[j] = 'the' + ' ' + num2words(int(str(seg[0])),ordinal = True) + ' ' + 'of' + ' ' + month_sim[str(seg[1])]+ ' ' + dateyear(seg[2][1:])
                                        else:
                                            output[j] = 'the' + ' ' + num2words(int(str(seg[0])),ordinal = True) + ' ' + 'of' + ' ' + month_sim[str(seg[1])]+ ' ' + dateyear(seg[2])
                                        output[j] = re.sub('-',' ',output[j])
                            else: 
                                if str(seg[2]).isdigit():
                                    output[j] = 'the' + ' ' + num2words(int(str(seg[2])),ordinal = True) + ' ' + 'of' + ' ' + month_sim[str(seg[1])]+ ' ' + dateyear(seg[0])
                                else:
                                    output[j] = 'the' + ' ' + num2words(int(re.findall("\d+", str(seg[2]))[0]),ordinal = True) + ' ' + 'of' + ' ' + month_sim[str(seg[1])]+ ' ' + dateyear(seg[0])
                                output[j] = re.sub('-',' ',output[j])
